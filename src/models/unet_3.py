'''Unet Model based on https://github.com/lil-lab/drif/blob/master/learning/modules/unet/unet_3.py
'''
import torch
...
class Unet3(torch.nn.Module):
  def __init__(self, in_channels, out_channels, embedding_size, hc1=32, hc2=16, k=5, stride=2):
    ...
    self.deconv2 = nn.ConvTranspose2d(
        2 * hc1, hc1, k, stride=stride, padding=pad)
    self.deconv3 = nn.ConvTranspose2d(
        2 * hc1, hc1, k, stride=stride, padding=pad)
    self.deconv4 = nn.ConvTranspose2d(
        2 * hc1, hc2, k, stride=stride, padding=pad)
    self.deconv5 = nn.ConvTranspose2d(
        hc1 + hc2, out_channels, k, stride=stride, padding=pad)
